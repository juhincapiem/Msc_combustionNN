...
for countTemp, temp in enumerate(temperatures):
    for countMFR, mfr in enumerate(massFlowRates):
            ...
            gas.TP = temp, 76000
            hGasInlet = abs(gas.enthalpy_mass)
            ...
            #eficiency = 1 - massFROutlet*hGasOutlet/(mfr*hGasInlet)
            eficiency = 1 - hGasOutlet/(hGasInlet)
...
df.to_csv('baseDeDatosV01.csv', index=False)
